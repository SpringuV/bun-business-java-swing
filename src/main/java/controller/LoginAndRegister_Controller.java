...
public class LoginAndRegister_Controller {
    private LoginAndRegister_Form DN_DkForm;

    public LoginAndRegister_Controller(LoginAndRegister_Form loginAndRegisterForm) {
        this.DN_DkForm = loginAndRegisterForm;
        listenActioner();
    }
    ...
    private void listenActioner() {
        ...
        DN_DkForm.getBtnToggle().addActionListener((e) -> {
            if (DN_DkForm.getAnim() != null && DN_DkForm.getAnim().isRunning()) return; // ignore khi đang animate
            if (DN_DkForm.isShowingLogin()) {
                slideIntro(true, () -> { // sau trượt xong -> đổi trạng thái & cập nhật text panel intro
                    ...
                    DN_DkForm.getTxt().setText("<html><div style='text-align: center; color:white; font-family: Serif; font-size: 13px;'>"
    ...
    public void slideIntro(boolean toLeft, Runnable onDone) {
        if (DN_DkForm.getAnim() != null && DN_DkForm.getAnim().isRunning()) DN_DkForm.getAnim().stop();
    ...
    public void animateFadeIn(FadePanel content, boolean fromLeft) {
        ...
        final int startContentX = fromLeft ? -(DN_DkForm.getINTRO_W() / 6) : (DN_DkForm.getINTRO_W() / 6); // bắt đầu hơi ở bên
